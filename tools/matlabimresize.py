...
def cubic(x, a=-0.5):
    ...
    f = np.multiply((a + 2) * absx3 - (a + 3) * absx2 + 1, absx <= 1) + np.multiply(
...
def imresize(I, scale=None, method='bicubic', shape=None, mode="vec"):
    ...
    if scale:
        ...
        scale = derive_scale_from_size(I.shape, shape)
    ...
    return B.astype(I.dtype)


def convert_double_to_byte(I):
    B = np.clip(I, 0.0, 1.0)
    B = 255 * B
